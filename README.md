...
| 功能 | 使用方式 |
|---|---|
| 菜单栏资源摘要 | 按已连接的平台显示数字：单平台如 `Q 16`，双平台如 `Q 16 · J 37`；未配置时显示应用图标 |
...
| 任务筛选与详情 | 勾选“仅运行中”快速筛选，也可在设置中按名称筛选；查看任务详情、复制任务 ID 或打开控制台 |
...
勾选“最近任务”右侧的“仅运行中”，即可查看所选平台中正在运行的任务；取消勾选后恢复显示所有状态。筛选先于最近 10 条的截取，较早启动的运行任务也能显示。应用记住勾选状态，任务通知仍按设置中的名称和资源范围监控。
...
点击菜单栏中的 GPUBar 图标，进入“设置”连接平台，并按需开启任务通知和登录启动。
...
启动后，点击菜单栏中的 GPUBar 图标，进入“设置”配置平台。应用可移至 Applications 使用。
